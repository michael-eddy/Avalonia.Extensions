...
namespace Avalonia.Extensions.Controls
{
    public class TextLabel : Runs, IStyling
    {
        ...
        /// </summary>
        public static readonly DirectProperty<TextLabel, TextRuns> TextProperty =
            AvaloniaProperty.RegisterDirect<TextLabel, TextRuns>(nameof(Children), o => o.Children, (o, v) => o.Children = v);
        /// <summary>
        /// Gets or sets the text.
        /// </summary>
        [Content]
        public override TextRuns Children
        {
            get => _items;
                ...
                switch (e.Action)
                {
                    ...
                    case NotifyCollectionChangedAction.Replace:
                        for (var i = 0; i < e.OldItems.Count; ++i)
                        {
                            ...
                            if (e.NewItems[i] is TextRun child)
        ...
        public override void Render(DrawingContext context)
        {
            ...
            {
                TextAlignment.Center => (width - TextLayout.Width) / 2.0,
                TextAlignment.Right => width - TextLayout.Width,
                _ => 0.0,
            };
            Thickness padding = Padding;
            double top = padding.Top;
            if (Bounds.Height < TextLayout.Height)
            {
                switch (VerticalAlignment)
                {
                    case VerticalAlignment.Center:
                        top += (Bounds.Height - TextLayout.Height) / 2.0;
                        break;
                    case VerticalAlignment.Bottom:
                        top += Bounds.Height - TextLayout.Height;
                        break;
                }
            }
            using (context.PushTransform(Matrix.CreateTranslation(padding.Left + offsetX, top)))
        ...
        internal virtual TextLayout CreateTextLayout(Size constraint, string text)
        {
            if (constraint == default)
                return null;
            return new TextLayout(text ?? string.Empty, new Typeface(FontFamily, FontStyle, FontWeight), FontSize, Foreground, TextAlignment, TextWrapping, TextTrimming,
                TextDecorations, FlowDirection, constraint.Width, constraint.Height, maxLines: MaxLines, lineHeight: LineHeight);
        }
